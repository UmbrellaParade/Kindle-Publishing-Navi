...
| `documentUrl` | 対応するGoogleドキュメントURL。空文字で未設定 |

`documentUrl`は`https://docs.google.com/document/...`だけを受け入れ、Googleスプレッドシート、Googleドライブの一般リンク、HTTP、なりすましホスト、認証情報・秘密queryを含むURLを拒否します。アプリはリンクを保存して新しいタブで開くだけで、Googleドキュメント本文を取得・同期・編集しません。
...
章ごとの`completed`と`documentUrl`も完全バックアップへ含めます。Googleドキュメントのファイル本体は含みません。同じ章IDの原稿進捗が結合元と結合先で異なる場合は、完成状態・リンクを静かに上書きせず、競合プレビューで停止します。異なる章IDの状態は和集合にします。明示的に未完成・URLなしへ戻した状態も有効な競合情報として扱い、古い値を復活させません。
...
- 章ごとの原稿完成状態は共有するが、`documentUrl`とGoogleドキュメントIDは共有JSON／Markdownの両方から除外する。共有Markdownの完成表示は現在の仮目次と現在の確定目次だけに重ね、過去版へ現在の進捗を表示しない
...
v6は、v5までの構造化保存、部・章・話・節の階層と紐づけ、編集中の仮目次、読み取り専用の確定目次と過去版、目次Markdownのプレビューと原子的な全面書き直し、旧ID・承認・取材リンクの退避、紐づけだけの再設定、並べ替え、版、承認、検索・絞り込み、市場調査正本Markdownのプレビュー取込、共有用書き出し、バックアップ互換に加え、構成項目ごとの原稿完成状態と非共有のGoogleドキュメントURL管理を対象とします。

...
- 既存プロジェクトで値がない場合は空のv5データとして扱う。保存済みv1／v2／v3／v4は読み込み時にv5へ安全に正規化する
...
旧v1／v2／v3／v4は、追加項目を未設定のままv5へ正規化します。v2までの平坦な章はID・本文・順序を維持したルートの「章」になり、v3／v4の階層目次もID・本文・親子関係・順序・章紐付けを変えず、全既存章IDを`draftOutlineChapterIds`へ入れて編集中の仮目次として保持します。既存データから確定目次を推測せず、`confirmedOutlineId` と `outlineSnapshots` は空で開始します。未知フィールド、未来のバージョン、無効な型、重複ID、存在しない構成項目IDや市場根拠IDへの参照は受け入れません。破損した値を空データとして上書きせず、復旧対象として扱います。
...
共有用JSONの識別子は `kindle-navi-planning-notes-share`、外側の`schemaVersion` は後方互換のため1を維持します。内側の`data.version`は5です。共有用Markdownはactiveな仮目次（編集中）、現在の確定目次、過去の目次（新しい順）、市場サマリー、正本、最初に見る資料、現行判断と履歴を含め、同じ内容を読みやすく変換します。退避台帳を現在の仮目次へ混ぜません。確定目次がない場合は明示し、仮目次と混同させません。
...
v5は、構造化保存、部・章・話・節の階層と紐づけ、編集中の仮目次、読み取り専用の確定目次と過去版、目次Markdownのプレビューと原子的な全面書き直し、旧ID・承認・取材リンクの退避、紐づけだけの再設定、並べ替え、版、承認、検索・絞り込み、市場調査正本Markdownのプレビュー取込、共有用書き出し、バックアップ互換を対象とします。

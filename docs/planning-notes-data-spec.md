# 企画・取材・構成ノート データ仕様（v7）

更新日: 2026年8月16日

## 目的と保存先

「企画・取材・構成ノート」は、企画、市場調査、章構成、取材、執筆指示書、意思決定、Kindle出版サポートGPTの世代管理を1冊ごとに構造化して保存する領域です。`PublishingProject.planning_notes` にJSON文字列として保存します。

- 既存プロジェクトで値がない場合は空のv7データとして扱う。保存済みv1／v2／v3／v4／v5／v6は読み込み時にv7へ安全に正規化する
...
旧v1／v2／v3／v4／v5／v6は、追加項目を未設定のままv7へ正規化します。既存のID・本文・順序・階層・章紐付けを変えず、GPT管理は空の`gptSessions`で開始します。未知フィールド、未来のバージョン、無効な型、重複ID、存在しない参照は受け入れません。破損した値を空データとして上書きせず、復旧対象として扱います。
...
章ごとの`completed`と`documentUrl`、`gptSessions`のURL・引継ぎメモを含む全フィールドも完全バックアップへ含めます。外部サービスのファイル本体は含みません。同じ章IDの原稿進捗が結合元と結合先で異なる場合は、完成状態・リンクを静かに上書きせず、競合プレビューで停止します。GPT管理は異なる内部IDを和集合にし、同じ内部ID・GPT管理IDの内容違いや複数の「使用中」が生じる場合は競合として停止します。
...
共有用JSONの識別子は `kindle-navi-planning-notes-share`、外側の`schemaVersion` は後方互換のため1を維持します。内側の`data.version`は7です。共有用Markdownはactiveな仮目次（編集中）、現在の確定目次、過去の目次（新しい順）、市場サマリー、正本、最初に見る資料、現行判断と履歴を含め、同じ内容を読みやすく変換します。GPT管理は全件除外し、空配列として書き出します。
...
v7は、v6までの構造化保存・目次履歴・原稿進捗・取材・指示書・意思決定・市場調査・バックアップ互換に加え、Kindle出版サポートGPTの世代・引継ぎ先・使用中の一意管理と、その情報を共有データから除外する非公開保護を対象とします。

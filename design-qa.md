...
- 変更前: `C:\Users\myabe\AppData\Local\Temp\codex-clipboard-ce0fa47e-a661-49e8-a565-9c791a4fd66f.png`
- 変更後（PC）: `C:\Users\myabe\AppData\Local\Temp\kindle-outline-rewrite-after-v1160.png`
- 変更後（スマホ）: `C:\Users\myabe\AppData\Local\Temp\kindle-outline-rewrite-mobile-final-v1160.jpg`

## 検品結果

1. 既存の個別削除では、本人承認済み・子階層・取材リンクの保護が初心者の全面改稿を妨げていた。
2. 仮目次の主要位置へ「目次をまとめて書き直す」を追加し、削除ではなく新しい仮目次への安全な切替として案内した。
3. Codex案のMarkdown貼付、部・章・話・節の階層プレビュー、消えるもの／残るものの確認を、3ステップで表示した。
4. 旧目次は読み取り専用履歴へ保存し、現在の確定目次・本人承認・取材の質問と原回答を変更しないことを実ブラウザで確認した。
5. 本人承認済み取材は「目次との紐づけだけ変更」で新しい話へ付け直し、承認状態と原回答が変わらないことを確認した。
6. 再読み込み後も、プロジェクト・新しい階層目次・過去の目次・新しい取材リンクが保持された。
7. PC 1920×1024では横溢れなし。スマホ390×844と320×568でも文書横溢れなし、ダイアログ内スクロール可能。
8. スマホのキャンセル／次へは44px以上。初期フォーカスはキャンセル、閉じた後は「目次をまとめて書き直す」へ復帰した。
9. PC／スマホのコンソール warning／error は0件。
10. 自動検証は297/297成功、lint、GitHub Pages相当production build、`git diff --check`も成功した。

## 最終比較

- 変更前の「削除してから失敗理由を知る」流れを、先に安全条件を示す一括書き直しへ置き換えた。
- 既存のダークテーマ、カード、シアン／ピンクの操作階層を維持し、主要CTAだけを独立した帯で強調した。
- 旧目次や承認済み取材を失わず、初心者が「貼る→確認→切替」の短い手順で全面改稿できる。
